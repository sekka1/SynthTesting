...
/******************************************** Security ***********************************************/
	class Security extends Base {
		...
		public function permCheck($permission, $requestedRights) {
                        if($this->user()->has_role("superuser")) {
                            // Role superuser has access to everything
                            return true;
                        }
			if($this->_oData["fakeAllPerms"]) {
		...
		public function loadAllPerms() {
			...
			$this->error("ERROR201207031231: loadAllPerms() has not been implemented");
                ...
                public function user() {
                    if(isset($this->_oData["user"])) {
                        return $this->_oData["user"];                        
                    } else {
                        $this->error("ERROR201210031435: When sequrity requested a user, one was not present.");
                    }                    
                }
