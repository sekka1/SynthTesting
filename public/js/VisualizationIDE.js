function visualizationIDE(visualizationID) {
    this.SDK = new AlgorithmsIO_SDK();
    this.myData = {};
    this.dirty = true;
    this.myid = visualizationID;
    ...
    this.setClean = function() {
        ...
        this.dirty = false;
        $('#button_saveVisualization').addClass("disabled"); // No need to save as we are clean
    }
    
    this.setDirty = function() {
        console.log("DEBUG201208241451: Setting Dirty");
        this.dirty = true;
        $('#button_saveVisualization').removeClass("disabled"); // Show Save Button
    }
    
    this.retrieve = function() {
        // Get the visualization data
        var algoIDE = this;
        return $.when(this.SDK.getVisualization(this.id())).then(function(result){algoIDE.updateFromObj(result);}).then(function(result) {algoIDE.setClean(); algoIDE.updateSaveTime(); algoIDE.previewRefresh.call(algoIDE); console.log("Done retrieve");});
    };
    ...
    this.updateFromObj = function(data) {
        ...
        if(typeof data["HTMLCode"] != "undefined") {
            this.setHTMLCode(data["HTMLCode"]);
        }
        if(typeof data["CSSCode"] != "undefined") {
            this.setCSSCode(data["CSSCode"]);
        }        
    ...
    this.save = function() {
        ...
        var afterSave = function(result) {
            ...
            if(algoIDE.id()) {
                ...
                if(result["id"]) {
                    ...
                    window.location.search = '?visualization_id='+result["id"];
                } else {
                    alert("ERROR201208241557: Saving of new visualization failed, as we did not get a result.");
                    console.log("ERROR201208241558: Saving a new algoirhtm did not return an ID:");
                    console.log(result);
                }
            }
            algoIDE.setClean();      
        ...
        $.when(this.SDK.saveVisualization(this.id(), this.myData)).then(afterSave,fail).then(algoIDE.updateSaveTime, fail);
        //$.when(this.SDK.saveVisualization(this.id(), this.myData)).then([afterSave, algoIDE.updateSaveTime]);
    ...
    this.setName = function(name) {
        ...
        $('#visualization_name').html(name); // Title at top
        $('#form_visualization_name').val(name); // Popup form
    }
    
    this.setDescription = function(description) {
        this.setDirty();
        this.myData["description"] = description;
        $('#form_visualization_description').val(description); // Popup form
    }
    ...
    this.setTags = function(tags) {
        ...
        $('#form_visualization_tags').val(tags); // Popup form
    }
...
function validateAlgorithmProperties() {
    ...
    if($('#form_visualization_name').val().length > 10) {
        // Should really do ajax to validate name hasn't been taken? - MRR20120820
        if($('#form_visualization_description').val().length > 20) {
            $('#button_properties_close').removeClass('disabled');
        }
    } 
}

function closeVisualizationProperties() {
    myIDE.setName($('#form_visualization_name').val());
    myIDE.setDescription($('#form_visualization_description').val());
    myIDE.setDetails($('#form_visualization_details').val());
    myIDE.setTags($('#form_visualization_tags').val());
    restoreIDEDiv($('#div_visualization_properties'));
